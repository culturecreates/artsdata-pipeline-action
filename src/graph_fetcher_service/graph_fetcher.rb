...
module GraphFetcherService
  class GraphFetcher
    ...
    def load_with_retry(entity_urls:)
      ...
      graph = @sparql.perform_sparql_transformation(graph, "remove_objects.sparql")
      graph = @sparql.perform_sparql_transformation(graph, "replace_blank_nodes.sparql", "domain_name", entity_urls[0].split('/')[0..2].join('/'))
      graph = @sparql.perform_sparql_transformation(graph, "fix_entity_type_capital.sparql")
      graph = @sparql.perform_sparql_transformation(graph, "fix_date_timezone.sparql")
      ...
      graph = @sparql.perform_sparql_transformation(graph, "fix_schemaorg_https_objects.sparql")
